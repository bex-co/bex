...
4. Re-diff the branding overlay (`lego/cli/internal/branding`). Confirm `cmd.CustomHelpTemplate` still contains `(eq .CommandPath "render")` (or update the string replace), `cliVersion` is still a cobra template func we can override, `docs` is still a root command we can retarget, and `SetupCommands` still late-adds commands so the HelpFunc re-walk remains necessary. Spot-check `bex --help`, `bex workspace set --help`, datastore command/flag help (including `pg`/`kv` aliases), and `bex blueprints validate --help` (`render.yaml` and `bex.yml` must stay intact). The workspace-set help override must match the bridge configuration precedence; flag usage and single-quoted workspace command references must remain Bex-branded.
5. Re-diff the telemetry sender seam (`pkg/analytics` + `internal/analyticsnotice`): the sender POSTs `CreateCliTelemetryEvent` to `<api-base>/cli-telemetry-events`, which bex-api implements (w5/m92) — confirm the endpoint path, the `CliTelemetryEventPOSTInput` field set (new fields must be accepted server-side; check `telemetryEventRequest`), the consent inputs (`RENDER_CLI_DISABLE_ANALYTICS`, `DO_NOT_TRACK`), and that the one-time notice marker path (`config.StateDir()/analytics/notice-shown`, i.e. `~/.render` unless `RENDER_CLI_CONFIG_DIR` is set) is unchanged. Record any schema delta in `docs/cli-compatibility-checklist.md`.
6. Run `bash scripts/bex-cli-validate.sh`, `cd lego/cli && go test ./...`, and the live device-flow check in `scripts/bex-cli-auth-e2e.sh` when Bex auth infrastructure is available.
7. Update `docs/cli-compatibility-checklist.md` with supported-version and compatibility evidence or an explicit Bex server-side limitation.
