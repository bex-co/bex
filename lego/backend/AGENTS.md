...
- `internal/api/` — composition root: one auth gate (`auth.go`) + one REST router / one GraphQL schema / one MCP registry. See [`internal/api/AGENTS.md`](internal/api/AGENTS.md).
...
Full meanings + defaults + ADR pointers live in the long descriptions below; this table is the quick lookup (operator/SSR vars → sibling `../operator/AGENTS.md`, `dashboard/AGENTS.md`). Unset ⇒ feature 503/disabled unless noted.
...
| bex-api | `BEX_RATE_LIMIT` `500`, `…_BURST`, `…_AUTH_FAILURE_LIMIT` `60`, `…_MAX_INFLIGHT` `64`, `…_MAX_INFLIGHT_PER_CREDENTIAL` `64`, `…_DEVICE_LIMIT` `30`, `…_WEBHOOK_LIMIT` `600`, `…_TRUSTED_PROXY_CIDRS` | token-bucket rates + trusted proxy CIDRs |
