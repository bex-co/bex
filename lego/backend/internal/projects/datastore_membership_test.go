...
func membershipFixture(t *testing.T) (*Service, client.Client) {
	t.Helper()
	...
	for _, member := range []struct{ name, tenant, project, environment string }{
		{"member", "tea-a", "prj-1", "env-member"},
		{"other", "tea-a", "prj-2", "env-other"},
		{"foreign", "tea-b", "prj-1", "env-member"},
	} {
		meta := metav1.ObjectMeta{Name: member.name, Namespace: "default", Labels: map[string]string{
			core.LabelTenant: member.tenant, core.LabelProject: member.project, core.LabelEnvironment: member.environment,
		}}
	...
	st := newFakeProjectStore(store.Project{ID: "prj-1", TenantID: "tea-a"}, store.Project{ID: "prj-2", TenantID: "tea-a"})
	return &Service{Base: base, Store: st, Databases: &postgres.Service{Base: base}, KeyValues: &keyvalue.Service{Base: base}}, cl
}
		...
		for _, invalid := range []string{"missing", "foreign", ""} {
			for _, mixed := range []bool{false, true} {
				t.Run(fmt.Sprintf("%s/id=%q/mixed=%t", kind, invalid, mixed), func(t *testing.T) {
					...
					_, err := set(ctxAs("user-a"), "prj-1", wanted)
...
func TestDatastoreMembershipAuthorizedEmptyReplacementClears(t *testing.T) {
	for _, kind := range []string{"postgres", "keyvalue"} {
		t.Run(kind, func(t *testing.T) {
			...
			if _, err := set(ctxAs("user-a"), "prj-1", nil); err != nil {
				t.Fatal(err)
			}
			...
			for i, previous := range before {
				...
				delete(want.GetLabels(), core.LabelProject)
