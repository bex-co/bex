...
At the end of the hunt, after bounded cleanup attempts and preserving any surviving-resource ledger, log out only this run's isolated device session, terminate owned processes/loopback servers, and remove owned temporary credentials/config/captures and browser state. Never remove pre-existing files, sessions, cookies, or credentials. Environment-token logout does not revoke that token; report any necessary remaining credential cleanup accurately. Cleanup instructions remain in force even when the user stops the loop.
