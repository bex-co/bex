...
**Estate enablement status (w7/m89) — held, gate off as of 2026-09-15.** Both correctness fixes above are live in production: the manager runs a digest built from `1cb1f2d27dda`, which contains `1343b7f17070` (m87) and `3aea3310212f` (m88). Capacity was re-checked the same day against the trial's admission budget and passes on the expected footprint — Zot at 41.49% of 98.31 GiB, retained-window peak 44.62%, leaving 20.03 GiB above that peak to the 65% ceiling, against an expected steady-state cache growth of 6–9 GiB (an analytic projection from the live catalog, not a measurement: `BEX_BUILD_CACHE` is manager-wide, so no per-App switch exists to measure a single production shape with). The all-3× worst case (~17.9 GiB) fits only without a concurrent rebuild transient and would rely on the trial's 70%-for-10-minutes stop rule rather than the 65% reserve. `BEX_BUILD_CACHE` therefore **remains unset in production**: the one outstanding start condition is human, not technical — the 48–72 hour observation window needs a named observer and an independently verified armed rollback. Numbers, thresholds, and the start/rollback procedure: [2026-09-09-build-cache-enablement.md](drills/2026-09-09-build-cache-enablement.md).
...
- **Serving overflow grows the stable pool instead**: `bex-tenant-0` is min1/**max3**, so tenant workloads and rollout surge can add a stable node while build nodes remain reserved for builds. The original max2 was raised on 2026-09-09 after both serving nodes filled their CPU/memory reservations: `eden-cms-v2` images built successfully, but each replacement pod stayed unschedulable and deploys ended `update_failed`. A third node provides room for the 1 CPU / 2 GiB replacement alongside the live revision; the autoscaler still owns replicas and can reclaim unneeded capacity.
