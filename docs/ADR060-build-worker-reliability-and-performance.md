...
- **It never re-arms, and it never decides the outcome.** Withdrawing is the safe direction — a trip costs a slow build. Re-enabling is a deploy, because a breaker that re-armed itself would oscillate the cache across a trial and make the very measurements it protects unreadable. Retaining the feature is a judgement about measured benefit that stays with a person.
