...
Original September 9 decision: **HOLD the global trial until the corrected operator is deployed and the production cache footprint fits the budget below.** The user approved a conditional 48–72 hour trial on September 9. The dated sections preserve the preflight and changing start conditions. **The bounded trial started September 17; see the final section for current status.**
...
## 2026-09-17 — bounded production trial started (w7/047)

The production manager started with `BEX_BUILD_CACHE=registry` at **2026-09-17T02:41:04Z**, after Argo reconciled `94246d5b9`. It runs the previously verified immutable image `sha256:8fa5d3964c5acbfa9375ccc99ce9254132065ba4052da6f760cae953a8aa91a0`, containing both correctness fixes and the automatic breaker. The committed deadline is **2026-09-20T02:34:30Z**, less than 72 hours after startup; initial review is due around **2026-09-19T02:41:04Z**. This supersedes the named-observer/manual-scheduler start requirement in the original procedure. The measured retain-or-rollback decision remains open.

Fresh preflight at 02:21 UTC measured Zot at **41.423/98.310 GiB (42.135%)**, retained peak **44.375 GiB (45.138%)**, and **19.527 GiB reserve above that peak to the 65% ceiling**. The retained window is about 3.1 days. Expected 6–9 GiB cache growth fits; it remains an analytic projection. Push p95 was **28.125s** over approximately 100 samples in 24h, with zero push errors and no active/queued builds.

The preflight also repaired the first-withdrawal alert: a reason-labelled counter first appears at 1, so `increase(...[1h])` misses the first trip. `BuildCacheTrialWithdrawn` now tests `bex_build_cache_withdrawals_total > 0`, with promtool tests for absent → 1, continued withdrawal after two hours, and staleness/reset. The root GitOps Application is blocked by an unrelated unmanaged analytics Secret; applying only the rendered Prometheus child Application installed the pending published alerts without modifying that Secret. `BuildCacheTrialWithdrawn`, `BuildPushTimeHigh`, and `ZotRegistryFillingUp` are all healthy and inactive in the live rule API.

Full operator `GOWORK=off make test`, all-module `make lint` including dead-code analysis, and the full GitOps validator passed. The optional FGA CLI drift check was unavailable locally. Latency/error withdrawal remains process-local and can reset on a manager restart; the absolute deadline does not. Cache-save failure is best-effort and does not directly enter the image-push failure counter. The PVC alert still requires a human response.

The disposable private App `srv-dall2gi9086jslvbntgg` (canonical ID, workspace `tea-d98210cbbpdc73dcrkvg`, the fixed public Node commit above) was created at 02:42:11Z. The live operator created `bld-srv-dall2gi9086jslvbntgg-gen-1` at 02:43:07Z; the Job completed and the App reached Ready. Cache restore was a cold miss (0s), cache save and image push each took 11s, and all phases exited zero. Independently reading Zot confirmed the previously absent image/cache repositories now held `gen-1` and `cache` tags. A loopback service forward returned HTTP 200 with `cache-trial-047`; no public Ingress was created. This proves activation and persistence, not a paired performance gain.

The Job ran for **46s**. Its cache referenced 409,141,163 compressed bytes, only 1,954 unique beside the image; this tiny fixture does not measure allocated PVC growth or the fleet footprint. UID-preconditioned deletion at **02:46:32Z** removed the App, its labelled Kubernetes resources, scoped Secrets, and both registry ACLs. Both repositories were independently confirmed empty at **02:48:13Z**. Blob reclamation follows normal GC. The final cache gauge was 1, with no withdrawal series, push errors, or firing Zot/Build alerts.
