...
1. Select the intended cluster with an explicit `KUBECONFIG`. Migrations `0113_cli_telemetry_events` and `0116_cli_telemetry_bex_version` must already be deployed, and `bex-system/bex-db` must have a ready primary. The view selects `bex_version`, so applying it before 0116 fails closed rather than serving a board without the release axis.
...
The telemetry-enabled source postdates the `bex-cli/v0.2.1` release. A source build or later telemetry-enabled release is required. Two version axes are now reported and filterable independently: the pinned upstream Render CLI version, and the bex launcher's own release (`X-Bex-CLI-Version`, w5/m94). A client that sends no release header -- an unmodified upstream `render` binary, or any build older than the header -- is recorded as `unknown`, which is a real population rather than a gap.

Opt-outs, missing authentication, early setup failures, sender backoff and delivery loss are unobserved; an unauthenticated run has no identity to attribute an event to, so it is silent by construction. On a fresh headless installation, delivery waits until the upstream one-time notice has been shown -- the two launcher-native paths below are the exception, because they run outside Cobra's post-run path where that gate lives. Login and logout remain non-events. Root version requests are now reported with completion kind `version`, and a successful `bex code` / `bex glm` / `muse` / `kimi` / `deepseek` launch reports itself immediately before it replaces the process (w5/m94); both carry only the command path, never the arguments handed to the launched agent. What the launched agent then does is the provider's session, not a bex command, and is never collected.
