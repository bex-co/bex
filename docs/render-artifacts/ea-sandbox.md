...
| (files, `ea sandboxes copy`) | `POST /v1/sandboxes/{sandboxId}/files/{operation}/token` (`upload`/`download`) then the returned `uri` | `{ "path": "…" }` | connect token / tar or octet-stream body — **not served by bex** (`404`; ledger gap) |
