...
A rolling update is zero-downtime by construction: `replicas: 1` gives `maxUnavailable: 25% → 0` and `maxSurge: 25% → 1`, so Kubernetes starts the new Pod and waits for `PodReady` before the old one is torn down. Production proved that is not sufficient, because of what the **surge Pod does to the node pool**.
