...
| **bex-api + gateways** | API error/latency from BOTH sides — Traefik's edge view per service and bex-api's own origin view per route pattern, surface, GraphQL operation, and MCP tool (w3/m84); agent-session SLOs (w5/m81 baseline, corrected w5/m88); the `/v1/sandboxes` surface, reported separately from agent sessions so either API's own regression is nameable (w5/m95); gateway saturation; outbound eventing delivery (webhooks + push) | Traefik series filtered to the api service; `bex_api_http_request_duration_seconds{surface,route,method,status}`, `bex_api_http_requests_total{…}`, `bex_api_http_in_flight_requests{surface}`, `bex_api_graphql_operation_duration_seconds{operation,type,outcome}`, `bex_api_mcp_tool_duration_seconds{tool,outcome}`; `bex_sandbox_create_total{outcome}`/`bex_sandbox_create_seconds`/`bex_sandbox_terminate_total{outcome}`; `bex_agent_session_turn_outcomes_total{outcome}`, `bex_agent_session_turn_duration_seconds{outcome}` (running-only; started_at→terminal), `bex_agent_session_provision_seconds{outcome}`, `bex_agent_session_terminal_convergences_total`; `bex_ssh_gateway_active_sessions`/`_active_channels`/`_limit_rejections_total`/`_git_proxy_upstream_failures_total`; `bex_webhooks_delivery_admissions_total{result}`, `bex_webhooks_delivery_attempts_total{origin,result}`; `bex_push_last_success_timestamp_seconds`, `bex_push_queue_rows{state}`, `bex_push_enabled` |
...
| **CLI usage** (product analytics) | observed adoption/repeat usage; command reach/failures/duration; detected agents/CI; environment mix; collection health | restricted Postgres `cli_analytics.events` view over retained `cli_telemetry_events`; Prometheus ingest-route request count/status/duration. The time range and command/upstream-version/bex-release/output filters scope product panels; collection health stays global. Two independent version axes are stored: the pinned upstream release and the launcher's own (`X-Bex-CLI-Version`, w5/m94). |
| **Product adoption** | inventory over time by resource type; distinct owning workspaces; explicit domains/ownership/TLS; observed changes; provisioning state, first-ready progress and unfinished age; creator/activation/deploy/cohort detail is collapsed | restricted `product-analytics` Postgres datasource over complete source snapshots and closed success facts. Unknown/stale collections are gaps, not zeros; inventory is separate from readiness. [Definitions and coordinated rollout](runbooks/product-analytics.md). |
...
| Sandboxes | `SandboxProvisionFailing`, `SandboxOrphansPresent`, `SandboxOldestOverBound` | — | **alert** — w5/m95 provision SLI; **w5/m99** inventory: `bex_sandbox_live{class}` / `bex_sandbox_oldest_age_seconds{class}` / `bex_sandbox_teardown_failures_total{reason}` keep a sandbox from outliving every session that owned it. Orphans warn after 2m; age past the 86400s lifetime bound warns after 15m |
...
So: **six hours is the nominal configured cadence, not a detection bound.** Do not restate it as "a regression surfaces within six hours", and do not promote the observed 7h28m maximum into a replacement promise — it is one sample of one workflow, and the other scheduled probes need their own evidence rather than an inference from this one. **This item supplies no independent freshness monitor**; [`w3/042`](../.pm/w3/done/042.md) owns the advisory cadence checker, and the evidence-based input for it is a freshness-alert threshold of **12h without a successful run** for this workflow — comfortably above the measured 7h28m worst case, and labelled as an alerting threshold rather than a promised bound. Moving the trigger in-cluster stays deferred; worsening drift can reopen it.

**The watchers are now watched (w3/042, 2026-09-23).** `ci-schedule-drift.yml` runs daily and keeps one `ci-schedule-drift` issue open while any scheduled workflow is outside its window: no verdict for longer than the window, a recovered gap longer than it in the last 48h, or no verdict at all in the runs inspected. Every scheduled workflow must declare a window in `scripts/ci-schedule-drift.sh` — `scripts (test)` fails otherwise — and each is set above that workflow's own measured worst gap: 12h for this one, 40h daily, 216h weekly. Two deliberate departures from the handoff above. A run counts once it reaches a **verdict**, success or failure, not only a success: this workflow's gaps between _successes_ reached 65h because its probes fail often, and a failing probe is `ci-red-streak`'s report. And **cancelled runs do not count**: `infra (terraform)`'s daily drift schedule fired every day from 2026-08-30 to 2026-09-09 and every run was cancelled, so nothing looked for eleven days while the schedule appeared healthy. Like `ci-red-streak`, it gates nothing.
