...
| Sandboxes | `SandboxProvisionFailing` | — | **alert** — w5/m95. `/v1/sandboxes` now has its own first-party series, so a regression confined to that surface (its authorization, template resolution, or the create verb itself) no longer hides behind the agent-session path. The two report separately on purpose: they share the OpenSandbox substrate, so both firing at once names the substrate, while either alone names its own API. Refusals decided before any runtime call are counted but excluded from the ratio, so a client sending invalid requests can neither raise a false alarm nor dilute a real one |
