...
Database teardown follows the same workspace boundary. [ADR086’s schema coverage contract](ADR086-account-deletion.md#schema-coverage-contract) requires every workspace-keyed SQL column to declare a cascade, registered purger, or deliberate retention. Its live-schema census follows the exact FK column through transitive cascades; namespace deletion alone cannot excuse retained database rows.
