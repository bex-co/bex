...
- **Authorization:** ADR088 states the fixture provisions real first-party production resources and is therefore an authorized human action, not something a scheduled job or an agent may do on its own. The user materialized this milestone through `/pm` on 2026-09-10 after the brainstorm flagged exactly that; that decision is the authorization for t001 under the operator's credentials. Every resource created is first-party, free-plan, billing-excluded, and recorded in ADR088. If the user prefers to run t001 personally, the worker starts at t002 with the handed-over ids.
