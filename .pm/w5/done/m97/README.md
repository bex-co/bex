...
**Worker:** worker5 **Goal:** every recorded resource creation carries a closed `surface` label — dashboard, CLI, MCP, direct API, or Blueprint — so the Product adoption board can show the agent-versus-human creation split ADR008's thesis makes the headline metric, with `unknown` as an honest bucket rather than an inference. **Status:** done — live-verified 2026-09-14 on production
...
**Live-verified 2026-09-14 (t006).** Production `product_activity_events.surface` and `product_analytics.events.surface` both present; `product_analytics.collection.surface_started_at=2026-09-11`. Last-14d creations already attributed: dashboard 46, api 24, cli 8, unknown 26 (legacy/`unknown` wedge). Panel SQL for "Creations by surface per day" and "Agent-driven share" execute against the live view (agent share 0 over the last 7d — no MCP creates in window, which is a real zero not a missing column). Grafana dashboard definition carries both panels; reader projection was re-bootstrapped with the surface column before this closeout.
