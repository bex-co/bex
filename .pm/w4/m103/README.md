...
## Adjacent lead — not filed, not verified

While answering the above, one asymmetry between the two paths surfaced and is worth a look during implementation rather than a board item of its own: the **service** path has no equivalent of `nextDatastoreAvailability`'s guard, so it appears to latch a first-observation `unhealthy` straight into the checkpoint. If it does, a brand-new service that is briefly unhealthy and then goes healthy would emit `server_available` ("recovered") as its first-ever event, despite never having failed — precisely the "spurious available edge" the datastore comment says it wants to avoid. **This was not verified**: `AvailabilityObserved` gating and the `rejectStaleUnhealthy` reference at `event_facts.go:274` may already prevent it. Check it while in this code; file separately if it is real.
...
- **Now observed — the "forever" framing is measured, not assumed.** Pass 8 (2026-09-13) re-ran the fixture and watched a second crash-looping first deploy (`srv-dajc7c26m8ac739r57ag`) for **14m42s past** its terminal `update_failed` (deploy closed 15:59:58; phase still `Deploying` on REST *and* GraphQL at 16:14:40, with no event emitted after `deploy_ended`). The 900s rollout budget had expired and bex-api's 18m deploy gate had already fired — it is what closed the row — so no timer remains that could revisit the phase. The full timeline is in t001 step 1, which no longer needs re-measuring. Pass 8 also recorded a timing caveat for tests: the close landed at 1091s after `build_ended` versus 898s in pass 5, so the transition drifts between the 900s budget and the 18m gate.
- ~~**Still unexplained:** no `server_failed` event …~~ **Explained 2026-09-13 pass 11 — by design, nothing to fix.** `server_failed` is an **availability** edge (`store/event_facts.go:443-449`, emitted only on `healthy → unhealthy`), so a deploy that was never healthy cannot produce one; the sibling datastore path documents the same rule and its reason (`store/datastore_event_facts.go:160-177`: never-Ready is "provisioning, not down", and latching would also cause "a spurious available edge … when provisioning finally succeeds"). A service that never served did not stop serving — its failure is the `deploy_ended / failed` fact, which did fire on both fixtures. Details in t001 step 2, which also **corrects a wrong hint** this milestone previously carried: the phase settle must not be hung off an availability edge, because that edge is designed never to fire here.
