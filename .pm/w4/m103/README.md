...
- **Still unexplained:** **no `server_failed` event was emitted for either crash-looping first deploy**, unlike a crash loop on a service with a prior release, which emitted `server_failed / readiness_failed` within ~40s during pass 4. Two independent fixtures now show the asymmetry, so it is reproducible rather than a one-off; t001 step 2 still owns understanding it, because the answer probably names the right hook for the phase fix.
