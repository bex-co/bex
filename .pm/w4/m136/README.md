...
- **The deploy-log panel is either proven complete or fixed.** t002 records a >100-line build's deploy logs with the exact GraphQL probe and response. If `hasMore` is ever `true` there, the panel pages or warns like the others.
...
- **Deploy-log truncation was not observed live.** No >100-line build was run this pass, so `use-deploy-logs.ts` is only reasoned to have the same gap (it never reads `hasMore`). That is t002's job, and it is deliberately not a DoD assertion yet.
