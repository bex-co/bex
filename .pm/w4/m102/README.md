...
- The env-var `anyOf` forms (`envVarFromDatabase` / `FromService` / `FromGroup` / `FromKeyValue` / literal) are named by the code comment as sharing this selection path, but no env-var misattribution was reproduced this run. t004 covers them as work to verify, not as an observed defect.
