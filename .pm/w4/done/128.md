...
Fixed. Every by-id verb now binds an optional `ownerId`, so a credential created in workspace B is readable, updatable and deletable in B:

- **GraphQL** (`registrycreds/graphql.go`) — `registryCredential(id:, ownerId:)`, `updateRegistryCredential(…, ownerId:)` and `deleteRegistryCredential(id:, ownerId:)` resolve through an `ownerScoped` helper, the same shape `w4/122` established for sandboxes. Omitting the argument keeps today's default-workspace behaviour, so no client breaks.
- **REST** (`registrycreds/rest.go`) — the note left this surface unchecked; it **had the same asymmetry**, and `GET`/`PATCH`/`DELETE /v1/registrycredentials/{id}` now bind `?ownerId` through `restOwnerScoped`.
- **MCP** — already correct. `mcpWorkspaceMiddleware` puts the named workspace in the context before the handler runs, so `WorkspaceOrDefault` was resolving the bound workspace, not a default.
- **`events.Get`** — the note's narrow instance. REST's `GET /v1/events/{eventId}` had **already** been given `?ownerId` since the note was written, so only GraphQL's `serviceEvent(id:)` was left; it now takes `ownerId` too, which is what a webhook subscriber needs to hydrate a delivered `data.id` belonging to a non-default workspace.
- **`clitelemetry`** — the note's last unchecked name. Its single `WorkspaceOrDefault` is on a **write** path (`clitelemetry/service.go:147`), stamping the caller's own acting workspace onto a telemetry row it is creating; there is no by-id read to mis-scope, so it is not an instance of this family.

Membership is enforced for free: `core.Base`'s named-workspace resolution runs `requireMember` for any workspace that is not the caller's own default (`core/base.go:651-668`), which is why the fix is an adapter binding and not a service-signature change.

Regression coverage, both mutation-checked (each fails with the binding removed):

- `TestGraphQLNonDefaultWorkspaceCredentialIsFullyReachable` — creates with `ownerId: tea-other`, then reads, updates and deletes it with `ownerId: tea-other`, and asserts the workspace's list is empty afterwards (the quota the `REGISTRY_CREDENTIAL_LIMIT` error tells the caller to free is actually freed). The control asserts a by-id read **without** `ownerId` still resolves the caller's default workspace, so the pass comes from the binding and not from a widened lookup.
- `TestRESTNonDefaultWorkspaceCredentialIsFullyReachable` — the same matrix across the three by-id routes, same control.
- `TestGetEventHonorsNamedWorkspace` (renamed from `…RESTHonors…`) gained the GraphQL half.

`TestCrossWorkspaceGQLMatrix`/`TestCrossWorkspaceMCPMatrix` still pass: their sweep deliberately omits `ownerId` arguments, so the new selectors cannot be used to widen a non-member's reach. Full `go test ./...` green in `lego/backend`.

Filed [`w4/140`](140.md) for the half this note did not cover: the **dashboard's** registry-credentials feature sends no `ownerId` on any of its five operations, so its page ignores the workspace switcher entirely — a wider, client-side defect that this server fix unblocks rather than causes.
