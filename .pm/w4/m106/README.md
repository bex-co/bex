...
- **Not examined:** whether any *mutation* has the same nesting shape (the collector treats Query and Mutation identically, so the question is whether a sensitive write hides under a write-classified root); env-group and datastore types were not swept for nested value fields — t002 owns that audit.
