#!/usr/bin/env bash
# Bootstrap Grafana's CLI-only reader on the selected CNPG cluster. Reuses its
...
if [ "${DRY_RUN:-0}" = "1" ]; then
  echo "would provision cli_analytics.events and restricted bex_cli_analytics role in $system_ns/bex-db"
  echo "would reuse or mint $monitoring_ns/grafana-cli-analytics (password + CNPG CA)"
  exit 0
fi
...
existing="$(kubectl -n "$monitoring_ns" get secret grafana-cli-analytics --ignore-not-found -o jsonpath='{.data.password}')"
...
kubectl -n "$system_ns" exec -i "$primary" -c postgres -- \
  psql -X -q -v ON_ERROR_STOP=1 -U postgres -d bex < "$script_dir/cli-analytics.sql"
{
  printf "\\set analytics_password '%s'\n" "$password"
  printf '%s\n' "ALTER ROLE bex_cli_analytics LOGIN PASSWORD :'analytics_password';"
} | kubectl -n "$system_ns" exec -i "$primary" -c postgres -- \
  psql -X -q -v ON_ERROR_STOP=1 -U postgres -d bex

apply_secret "$monitoring_ns" grafana-cli-analytics Opaque password "$password" ca.crt "$ca"
unset password ca existing
echo "provisioned cli_analytics.events and $monitoring_ns/grafana-cli-analytics"
